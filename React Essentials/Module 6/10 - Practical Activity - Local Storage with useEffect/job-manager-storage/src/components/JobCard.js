...
// Shows one job's activity, categories and time added
function JobCard({ job, deleteJob, onEdit, isEditing, formatTime }) {
  // Bonus 3: remember which job is being dragged
  ...
  return (
    <div className={isEditing ? 'job-card job-card--editing' : 'job-card'} draggable onDragStart={handleDragStart}>
      <h3>{job.activity}</h3>
      {/* Bonus 2: when the job was added */}
      {job.createdAt && <p className="job-card__time">Added {formatTime(job.createdAt)}</p>}
      ...
      <div className="job-card__buttons">
        ...
        {/* The delete button calls the function passed down from JobManager */}
