...
Open any `index.html` (or `home.html`) in a browser. Pages that use Google Fonts, Font Awesome, Bootstrap, Tailwind's Play CDN or Google Maps need an internet connection to look right.

To preview through a local server instead, run this from the repo folder, then open `http://localhost:8000`:

```text
python3 -m http.server 8000
```

For the npm Tailwind project, run `npm install` inside `Tailwind CSS/Module 4/NPM Tailwind` first, then `npm run build`.
